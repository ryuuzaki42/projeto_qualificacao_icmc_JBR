...
% \ideia{caixa retangular de fundo verde e texto preto}
% \duvida{caixa retangular de fundo vermelho e texto preto}
% \rev{texto em vermelho com parentese em volta}
...
\curso{CCMC}
\data{26}{02}{2018} % Data do depósito
...
\incluifichacatalografica{packages/ficha-catalografica-JBR.pdf}
...
Atualmente as atividades de trânsito, o transporte de mercadorias e de passageiros, são importantes no dia a dia das pessoas e são essenciais para o desenvolvimento humano. Na execução destas atividades, a direção/condução é um dos fatores mais agravantes com a segurança no trânsito. Assim é importante um meio que ofereça suporte ao motorista e propicie uma maior segurança nas rodovias \cite{Fuentes-2011}.
...
\ideia{O objetivo principal desta dissertação será abordado ao respondermos as questões derivadas que seguem:

\begin{itemize}
    \item \textbf{os veículos apresentam movimento periódico (e.g.,  de casa para o trabalho e vice-versa) e limitados a rodovia. Esses movimentos ou padrão de mobilidade aliado com as categorias dos veículos poderiam de certa forma influenciar no encaminhamento de mensagens?} Nosso intuito aqui é analisar se o padrão de mobilidade de cada categoria dos veículos poderia ser utilizado de modo a otimizar o encaminhamento de mensagens. Assim respondendo a pergunta, é melhor \aspas{ver} os veículos como iguais ou as diferenças entre um táxi, caminhão e ônibus (por exemplo) podem ajudar no encaminhamento de mensagens.

    \item \textbf{Existe uma maneira simples e rápida de identificar cada veículo a sua respectiva categoria e assim fazer uso desta informação?}
    
    \item \textbf{Quais e quantas categorias são relevantes diferenciando entre si os padrões de mobilidade?}
    
    \item \textbf{Existem outros usos onde a categorias dos veículos, obtida de maneira rápida e prática, poderia ser útil?}
    
    \item \textbf{Riscos em relação a segurança e uso das categorias de veículos?}
    
    \item \textbf{Ganhos em relação a segurança e a utilização das categorias de veículos?}
\end{itemize}
}
...
Em 1999, a FCC \footnote{Federal Communications Commission}, com um dos primeiros esforços para padronização das redes veiculares nos Estados Unidos, alocou 75 MHz do espectro de frequências na faixa de 5,9 GHz (5,850-5,925 GHz) para aplicações DSRC (\autoref{figure:canais_dsrc}). A faixa é livre, porém licenciada, ou seja, não é cobrado uma taxa pela utilização, mas definem quais as aplicações e tecnologias a serem utilizadas. Em outros países foram definidas outras faixas de frequência \cite{Alves-2009,li-2012,kenney-2011}.

O DSRC foi projetado em um sistema multicanal, com espectro de frequência dividido em sete canais (\autoref{figure:canais_dsrc}). Cada canal tem a largura de banda de 10 MHz, seis deles são definidos como \sigla{SCH}{\textit{Service Channel}} e um como \sigla{CCH}{\textit{Control Channel}}. A faixa de banda 5,850-5,855 GHz é reservada (\textit{guard frequency}). O canal de Controle é utilizado para mensagens de segurança, enquanto os Canais de Serviços são utilizados para aplicações e serviços \textit{non-safety} \cite{sattari-2012,li-2012}.
...
O SAE J2735 define 15 tipos de mensagens, a maioria delas têm em seu escopo a identificação do veículo, dados de localização e o \textit{timestamp}. Um exemplo destas mensagens é a \sigla{BSM}{\textit{Basic Safety Message}}, que transmite informações sobre o estado (e.g., velocidade e localização) do veículo para veículos próximos. Um elemento de dado é uma informação obtida por algum sensor presente no veículo (e.g., velocidade) e por conceito é indivisível. O agrupamento de um conjunto de elementos de dados forma um quadro de dados. Aliada a SAE J2735, existe a SAE J2945.1 \footnote{SAE J2945.1 DSRC Minimum Performance Requirements}, que especifica os requisitos mínimos de desempenho das comunicações DRSC definidas pela SAE J2735 \cite{li-2012,grilli-2010,kenney-2011}.
...
De posse das informações de estado dos veículos próximos, é necessário uma aplicação para gerenciá-las e processá-las, apresentando para o motorista avisos importantes sobre a cinemáticas dos veículos próximos. Com esse sistema o motorista pode prever melhor a cinemática dos outros veículos e assim ter uma maior consciência dos acontecimentos e consequentemente propiciar uma maior segurança no trânsito. Os \textit{beacons} são úteis para protocolos de roteamento, pois possibilitam a descoberta de um possível \rev{bom veículo} próximo para encaminhar uma determinada mensagem  \cite{noori-2013, Nguyen-2002,Fuentes-2011}.

Nas VANETs, os \textit{beacons} precisam ser enviados frequentemente, por serem redes altamente dinâmicas. Entretanto, um frequente envio de \textit{beacons} pode degradar significativamente o desempenho de uma rede altamente densa, devido a muitas colisões de quadros (\textit{frames}). Por outro lado, a baixa taxa de envio de \textit{beacons} pode atrasar a percepção dos veículos próximos e também reduz a confiabilidade dos protocolos com requisitos de tempo de entrega. Desse modo, a frequência ideal de envio dos \textit{beacons} pode variar dependo da quantidade de veículos e aplicação em uso. \textit{Beaconing} é utilizado em muitas aplicações para VANETs \cite{Nguyen-2002,Thaina-2011,noori-2013}
...
Veículos de diferentes tipos apresentam distintos padrões de mobilidade, cada qual com suas peculiaridades. Pode-se classificar os veículos em algumas categorias, tais como táxi, ônibus, veículo particular ou de passeio e veículos pesados (caminhões) para transporte de carga. Os ônibus têm uma cobertura espacial e temporal limitada (e.g., um ônibus que em determinado horário tem como rota vários bairros da cidade e depois volta para a rodoviária). Desse modo, os ônibus movem-se ao longo de rotas fixas durante um determinado período de tempo, o que implica em um forte padrão de mobilidade. O que pode ser muito útil para o roteamento, dado que a mobilidade dos ônibus (nós na rede) é de certa forma previsível. Dada uma área de destino de uma mensagem, os ônibus cujas rotas cruzar com essa área devem ser preferidos para o roteamento da mensagem \cite{Zhang-2014,Zhang-2012}.

O padrão de mobilidade dos táxis é mais diversificado do que o dos ônibus, com uma maior cobertura espacial e temporal. A mobilidade dos táxi é afetada por dois fatores, as demandas dos clientes e os hábitos de condução do motorista (e.g., um táxi tem como destino o centro da cidade, entretanto o motorista não passará pela avenida principal na tentativa de evitar congestionamentos). Se um táxi está ocupado com algum cliente, a mobilidade é determinada, principalmente, pelo destino do cliente. O motorista pode escolher o caminho mais curto e/ou com menor congestionamento. Se o táxi não está ocupado, a mobilidade depende dos hábitos de condução do motorista e da sua preferência por lugares para buscar por novos clientes \cite{Zhang-2012,Zhang-2014}.
...
\textcolor{red}{add +  algoritmos SAR e VADD e mais outros}
